...
const ManagerDashboard = () => {
  ...
  const handleStatusUpdate = async (bookingId, newStatus) => {
    try {
      ...
      setSales(
        sales.map((sale) => (sale.id === bookingId ? response.data : sale)),
      );
    } catch (err) {
      alert(
          ...
          <tbody>
            {sales.map((sale) => (
              <tr key={sale.id}>
                ...
                <td>
                  <small>
                    {sale.check_in_date} to {sale.check_out_date}
                  </small>
                </td>
                    ...
                    {sale.status === "PENDING" && (
                      <>
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleStatusUpdate(sale.id, "CONFIRMED")
                          }
                        >
                          Confirm
                        </Button>
            ...
            {sales.length === 0 && (
              <tr>
                <td colSpan="7" className="text-center py-5 text-muted">
                  No sales yet. Make sure your inventory is set to "Active" so
                  travelers can book it!
                </td>
              </tr>
            )}
          </tbody>
